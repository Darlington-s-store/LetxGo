...
export const EmailVerificationPageNew = ({
  email,
  onVerifyCode,
  onNavigateBack,
  onResend
  ...
  const maskedEmail = email
    ? `${email.substring(0, 3)}${'*'.repeat(email.split('@')[0].length - 3)}@${email.split('@')[1]}`
    : '';
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <Logo />
            <h1 className="text-3xl font-bold text-gray-900 mt-6 mb-2">Email Verification</h1>
            <p className="text-gray-600 text-sm">
              Enter the verification code we just sent on your email address<br />
              <span className="text-gray-700 font-medium">{maskedEmail}</span>
            </p>
          </div>
